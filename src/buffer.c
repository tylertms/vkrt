...
VkDeviceAddress appendBufferFromHostData(VKRT* vkrt, const void* hostData, VkDeviceSize appendSize, VkBufferUsageFlags usage, VkBuffer* ioBuffer, VkDeviceMemory* ioMemory, VkDeviceSize oldSize) {
    ...
    void* mapped = NULL;
    vkMapMemory(vkrt->device, stagingNewMem, 0, appendSize, 0, &mapped);
    memcpy(mapped, hostData, (size_t)appendSize);
    vkUnmapMemory(vkrt->device, stagingNewMem);
    ...
    VkCommandBuffer cmd;
    vkAllocateCommandBuffers(vkrt->device, &commandBufferAllocateInfo, &cmd);
    ...
    vkFreeCommandBuffers(vkrt->device, vkrt->commandPool, 1, &cmd);

    vkDestroyBuffer(vkrt->device, stagingNewBuf, NULL);
    vkFreeMemory(vkrt->device, stagingNewMem, NULL);

    vkDestroyBuffer(vkrt->device, *ioBuffer, NULL);
    vkFreeMemory(vkrt->device, *ioMemory, NULL);
