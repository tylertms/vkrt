...
int vkrtOIDNDenoise(const VKRT_OIDNFilterInput* input, float* output, const char** outErrorMessage) {
    ...
    const size_t imageByteCount = rowStride * (size_t)input->height;

    if (!createOIDNBuffer(device, input->color, imageByteCount, &colorBuffer, outErrorMessage, "color")) {
        goto cleanup;
    }

    outputBuffer = oidnNewBuffer(device, imageByteCount);
    if (!outputBuffer) {
    ...
    if (input->albedo &&
        !createOIDNBuffer(device, input->albedo, imageByteCount, &albedoBuffer, outErrorMessage, "albedo")) {
        goto cleanup;
    }
    if (input->normal &&
        !createOIDNBuffer(device, input->normal, imageByteCount, &normalBuffer, outErrorMessage, "normal")) {
        goto cleanup;
    }
    ...
    oidnSetFilterBool(filter, "cleanAux", input->cleanAux ? true : false);
...
cleanup:
    releaseOIDNResources(&device, &filter, &colorBuffer, &albedoBuffer, &normalBuffer, &outputBuffer);
    return succeeded;
#endif
}
