...
VKRT_Result createCommandPool(VKRT* vkrt) {
    ...
    if (vkCreateCommandPool(vkrt->core.device, &commandPoolCreateInfo, NULL, &vkrt->runtime.commandPool) !=
        VK_SUCCESS) {
...
VKRT_Result createCommandBuffers(VKRT* vkrt) {
    ...
    if (vkAllocateCommandBuffers(vkrt->core.device, &commandBufferAllocateInfo, vkrt->runtime.commandBuffers) !=
        VK_SUCCESS) {
